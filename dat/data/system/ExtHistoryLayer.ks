...
class HistoryMessageLayer extends Layer
{
	...
	var marginL = 40;		// 履歴メッセージ領域左余白
	var marginR = 40;		// 履歴メッセージ領域右余白
	var marginT = 12;		// 履歴メッセージ領域上余白
	var marginB = 12;		// 履歴メッセージ領域下余白
	var fontName = "MS Gothic";	// 履歴メッセージdefaultフォント名
	var fontBold = false;		// 履歴メッセージdefault太字細字
	var fontHeight = 14;		// 履歴メッセージdefaultフォントサイズ
	var lineHeight = 20;		// 履歴メッセージ行幅
...
class ExtHistoryLayer extends Layer
{
	...
	var closegraphic = "ボタンx";		// closeボタンの画像ファイル名
	...
	var scrollbarHeight = 32;	// スクロールバー幅(画像btnなら未使用)
	...
	function onMouseDown(x, y, button)
	{
		...
		//if (button == mbRight) // 左クリックでも閉じられるようにした
