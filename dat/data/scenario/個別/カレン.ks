...
[jump target="*カレンデート１" cond="f.カレンデート == 1"]
[jump target="*カレンデート２" cond="f.カレンデート == 2"]
...
[jump target="*人が集まらない休日準備2" cond="!足跡('人が集まらない休日準備') || 足跡('ちょっと思いつかないなあ')"]
...
[種入手 名前=カレンの本音 内容=カレンはマユに、本当は参加してほしいと思っている]
[足跡 名前=カレンの本音]
[自分]
...
;ルートを完遂した場合は発生しません。
[jump target="*また図書室で2" cond="足跡('和菓子屋とペットのこと')"]
[カレンなにしてる]
...
[足跡デバッグ 名前=楽しく参加するには]
[jump target="*楽しく参加するには" cond="!足跡('楽しく参加するには')"]
...
[if exp="種がある"]
	[next]
[else]
	[eval exp="var 呼ぶ = '放課後の文化祭準備に呼んでもらう (' + 招集消費 + ')'"]
	[二択 ア=&呼ぶ イ=呼ばない]
...
-----------------------------[endnowait][next]
[足跡 名前=和菓子屋とペットのこと]
[種入手 名前=カレンとデート 詳細=カレンと動物園でデートすることになった 期限=1]
[eval exp="f.カレンデート = 2"]
[return]
...
-----------------------------[endnowait][next]
[eval exp="f.カレンデート = 0"]
[足跡デバッグ 名前=俺の方こそ楽しんでいるんだ]
[種分岐 ア=俺の方こそ楽しんでいるんだ イ=何を言えばいいのか…… 体力=&告白消費]
[if exp=ア]
	[eval exp="f.カレン攻略 = 1"]
	[足跡 名前=カレン攻略]
	カレンを攻略した。[next]
