...
[枠消去]
[二択 ア=攻略のヒントを聞く イ=聞かずにタイトルへ]
[if exp=イ][タイトルへ戻る][endif]
[BGM停止]
[場面転換]
[背景 画像=教室][枠]
[BGM 曲=日常昼.ogg 音量=50]
...
うむ。[next]
[人物左 画像=カレンねえ 名前=恋ヶ窪カレン]
まずは、進捗率 [emb exp="f.進捗"]％ これはどうなのかしら。[next]
[表情左 画像=カレン真顔]
[if exp="f.進捗 < 60"]
	[人物右 画像=ヨシオ困惑 名前=小平ヨシオ]
	これは……[n]
	ひょっとして[姓]、進捗を遅らせることに全力を注いだのでは？[next]
	[表情右 画像=ヨシオ]
	[自分]
	いや、そんなわけじゃ……[next]
	[表情右 画像=ヨシオ説明]
	休日はきちんと準備をしないと、文化祭には間に合わない。[n]
	他にも方法はあるかもしれないが、これが基本だ。[next]
	[表情右 画像=ヨシオきりっ]
	[自分]
	わかったよ……[next]
[elsif exp="f.進捗 < 70"]
	[人物右 画像=ヨシオしゃべる 名前=小平ヨシオ]
	補習を食らったり、招集をしなかったり、休日に何もしないなど、[n]
	いろいろアクシデントが重なったように思えるな。[next]
	[表情右 画像=ヨシオ説明]
	我々３人がフルに頑張って、70％ くらいが限界だ。[n]
	誰かが参加してくれるとしたら、[n]
	状況は好転するかもしれないのだが。[next]
	[表情右 画像=ヨシオ自信]
	あのクラスを見たところ、[n]
	本多が一番、参加へのハードルが低いように見えるな。[next]
	[表情右 画像=ヨシオきりっ]
	[自分]
	なるほど、本多さんか。[next]
[elsif exp="f.進捗 < 80"]
	[人物右 画像=ヨシオしゃべる 名前=小平ヨシオ]
	文化祭を成功させるには２つの方法があると僕は考える。[next]
	[表情右 画像=ヨシオ説明]
	ひとつは、クラスメイトを臨時で招集する、[n]
	もうひとつは、説得して参加をしてもらうという方法だ。[next]
	[表情右 画像=ヨシオ自信]
	あのクラスを見たところ、[n]
	本多が一番、参加へのハードルが低いように見えるな。[next]
	[表情右 画像=ヨシオきりっ]
...
[elsif exp="f.進捗 < 90"]
	[人物右 画像=ヨシオしゃべる 名前=小平ヨシオ]
	ずいぶんと惜しいところまで来たのではないかと僕は考える。[next]
	[表情右 画像=ヨシオ説明]
	もし早いうちから５人で作業出来たら、[n]
	文化祭の成功は圏内であっただろうな。[next]
	[表情右 画像=ヨシオ自信]
	梶野カケルと美園ミドリ、あの二人が参加してくれれば[n]
	我々と同等か、それ以上に仕事をしてくれるはずだ。[next]
	[表情右 画像=ヨシオきりっ]
	[自分]
	あの二人ってそんなにすごいのか……[next]
[else]
	[人物右 画像=ヨシオしゃべる 名前=小平ヨシオ]
	勝利は目前であったが、実に惜しかった。[next]
	[表情右 画像=ヨシオ説明]
	勉強に力を入れ過ぎていないか、[n]
	昼休みに行動しすぎて余分な体力を使っていないか、[n]
	異性交遊にうつつを抜かしていないか……[next]
	[表情右 画像=ヨシオ自信]
	[姓]、おそらくこの先は、いかに無駄を省けるかという問題だ。[next]
	[表情右 画像=ヨシオきりっ]
	[自分]
	無駄を減らす……か。[next]
[endif]
[if exp="f.イズミ不参加 + f.マリ不参加 + f.ミドリ不参加"]
	[if exp="f.イズミ不参加"]
		[人物左 画像=カレンねえ 名前=恋ヶ窪カレン]
		本多さんが参加してくれなくなったのって、[n]
		私と喧嘩したからよね。[next]
		
		あれ、[姓]くんの行動次第で防げたかもしれないわね。[next]
		[表情左 画像=カレン真顔]
		[自分]
		え？ 俺のせいなの！？[next]
	[endif]
	[if exp="f.マリ不参加"]
		[人物左 画像=カレンねえ 名前=恋ヶ窪カレン]
		[emb exp="'それに、'" cond="f.イズミ不参加"]
		貫井さんが参加してくれなくなったのって、[n]
		[姓]くんが秘密をバラしたからでしょう？[next]
		
		あれ、どう見ても１組の国分寺くんの罠だと思うの。[next]
		[表情左 画像=カレン真顔]
		[自分]
		えええ？ カズキはそんなヤツじゃないよ！[next]
	[endif]
	[if exp="f.ミドリ不参加"]
		[人物左 画像=カレンねえ 名前=恋ヶ窪カレン]
		[emb exp="'あと、'" cond="f.イズミ不参加 + f.マリ不参加"]
		美園さんが参加してくれなくなったのって、[n]
		[姓]くんがクラスの女の子に梶野くんのことを[n]
		吹き込んだからって聞いたけど……[next]
...
[elsif exp="最終人員 < 5"]
	[人物左 画像=カレン哀愁 名前=恋ヶ窪カレン]
	今回はみんなとあまりお話はしなかったけど、[n]
	もうちょっと頑張ってみるのも良かったかもしれないわね。[next]
	[表情左 画像=カレン]
	[自分]
	うん、そうだね。[next]
[else]
	[人物左 画像=カレン哀愁 名前=恋ヶ窪カレン]
	せっかくたくさんの人が手伝ってくれたのだから、[n]
	もう少し早く参加してくれればね。[next]
	[表情左 画像=カレン]
	[自分]
	うん、そうだね。[next]
[endif]
...
つらいこと、苦しいことに出会っても、貫井さんと一緒なら、[n]
いつだって真っすぐ乗り越えて行けそうな気がする[ま][ま][n]
...
[人物左遠い 画像=カレンにこ 名前=恋ヶ窪カレン]
本当に反省しているんだったら、[n]
神様が奇跡を起こしてくれると思わない？ ねえ、マリ。[next]

[人物右遠い 画像=マリあはは 名前=貫井マリ]
あー、いいね！ それ。[next]
...
[macro name=間スタッフ]
[wait time="&waitstaff / 5" canskip="false"]
