...
[if exp="f.説明 == 'キー操作'"]
	...
	[人物 画像=前原先生 名前=操作ガイド]
	画面の見方を説明します。[next]
	[image layer="&lay説明 + 1" storage="説明日付" visible="true"]
	これは文化祭までの日数です。[n]
	１週間は、[font color=&色：強化]平日１ → 平日２ → 休日[resetfont] の３日の繰り返しです。[next]
	...
	これはバックログ表示ボタンです。[n]
	シナリオの文章を読み直すことができますが、[n]
	前の位置からの再生はできません。[next]
	[layopt layer="&lay説明" opacity="255"]
	[image layer="&lay説明 + 1" storage="説明授業" visible="true"]
	授業の受け方を選ぶことができます。[n]
	たとえば真面目に授業を受けると、体力を [font color=&色：強化]15[resetfont] 消費します。[next]
	
	授業中に寝て体力を回復することもできますが、[n]
	徐々に成績が落ちていくので注意が必要です。[next]
	[layopt layer="&lay説明 + 1" visible="false"]
	[layopt layer="&lay説明" opacity="191"]
	これで説明を終わります。[n]
	まずは、授業を真面目に受けてみましょう。[next]
	[nowait]
	[人物 画像=前原先生しゃべる 名前=古典の前原]
	鈴虫の 声の限りを 尽くしても 長き夜明かず ふる涙かな[r]
...
[if exp= "f.日 == 17 || f.日 == 11 || f.日 == 5 || f.日 == 2"]
	...
	[if exp= "f.日 >= 11"]
		次のテストは [font color=&色：強化]２週間後[resetfont] だから、また頑張って。[next]
	[elsif exp="f.日 == 5"]
		次のテストは [font color=&色：強化]来週[resetfont] か……[n]
		文化祭前で大変だとは思うが、頑張って。[next]
	[else]
		次のテストは……[n]
		そうか、文化祭の後か。[next]
		
		文化祭が終わったら、またお知らせがあるからね。[next]
	[endif]
	[人物消去]
	[メッセージ]
	やっとおわった。[n]
...
[if exp= "f.日 == 16 || f.日 == 10 || f.日 == 4 || f.日 == 1"]
	...
	[モノローグ]
	[if exp="f.試験の点数 < 50"][やらかした.ogg][endif]
	...
	[cancelskip]
	[next]
	[if exp="成績優秀"][種入手 名前=成績優秀！ 詳細="&'テストで' + f.試験の点数 + '点を取ったぞ'" 期限=3][endif]
	[if exp="実績主席"][実績解除 名前=実績７：学年主席][endif]
	[if exp="実績水平"][実績解除 名前=実績８：赤点水平飛行][endif]
	[モノローグ終了]
[endif]
...
[if exp= "f.日==20"]
	[人物 画像=女性教員 名前= "英語のシエ=サン"]
	[font color=&色：強化]来週、テスト[resetfont]。[間]忘れずに頑張ってクダサーイ。[n]
	５０点で放課後スッペショーレッスン！ Good luck！[next]
	[人物消去]
	...
	なんとしても [font color=&色：強化]５０点[resetfont] は死守するんだ。[next]
	[モノローグ終了]
[endif]
